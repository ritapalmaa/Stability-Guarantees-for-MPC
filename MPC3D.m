...
if obst_flag == 1
    Obs.G = diag([1 1 1 200 200 200 200 200 200]); Obs.c = zeros(nx,1); 
...
v_min = 3; v_max = 2*v_min; % limits to search for a initial condition
...
%     [vertex, ~] = confirmation{vector};
    ...
    if vertex ~= zeros(nx,1) 
        ...
        if flag ~= 0 
            disp('Not in RA.')
            subplot(1,3,1); plot3(x0(1),x0(2),x0(3),'*','LineWidth',1);
            subplot(1,3,2); plot3(x0(4),x0(5),x0(6),'*','LineWidth',1);
            subplot(1,3,3); plot3(x0(7),x0(8),x0(9),'*','LineWidth',1); 
        end
...
%         idx = FindSet(Xj_list,x0);
        ...
        path = MPCSimulation(controller,x0,Nc,model,Xj_list,N,terminal,set_max);
